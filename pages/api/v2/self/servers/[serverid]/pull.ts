...
async function handler(
    ...
    try {
        ...
        if (!serverId)
            return res
                .status(400)
                .json({ success: false, message: "Server ID not provided" });
        if (!guildId)
            return res
                .status(400)
                .json({ success: false, message: "Server not provided" });
        if (pullCount === undefined || pullCount === null)
            pullCount = Number.MAX_SAFE_INTEGER;

        const server = await prisma.servers.findFirst({
            where: {
                AND: [{ guildId: BigInt(serverId) as bigint }, { ownerId: user.id }],
            },
        });
        if (!server)
            return res
                .status(400)
                .json({ success: false, message: "Server not found", code: 50041 });
        ...
        const bot = await prisma.customBots.findFirst({
            ...
            where: {
                AND: [{ ownerId: user.id }, { id: server.customBotId }],
            },
        });
        if (!bot)
            return res
                .status(400)
                .json({ success: false, message: "Bot not found", code: 50042 });
        ...
        if (members.length === 0)
            return res
                .status(400)
                .json({ success: false, message: "No members found", code: 50043 });
        ...
        await axios
            .get(`https://discord.com/api/v10/users/@me`, {
                ...
                httpsAgent: new HttpsProxyAgent(
                    `https://${process.env.PROXY_USERNAME}:${process.env.PROXY_PASSWORD}@zproxy.lum-superproxy.io:22225`
                ),
                validateStatus: () => true,
            })
            .then((response) => {
        ...
        await axios
            ...
            .then((resp) => {
                if (resp?.status !== 200 || resp?.status != 200)
                    return res.status(400).json({
            success: false,
            message: "Discord server not found, invite bot or try again.",
            code: 500404,
          });
                console.log(
                    `[${server.name}] Pulling members into server: ${resp?.data?.name} (${resp?.data?.id})`
                );
        ...
        if (roleId) {
            await axios
                .put(
                    `https://discord.com/api/v10/guilds/${guildId}/members/${bot.clientId}/roles/${roleId}`,
                    {},
                    {
                        headers: {
                            Authorization: `Bot ${bot.botToken}`,
                ...
                .then((resp) => {
                    if (resp?.status === 403 || resp?.status == 403)
                        return res.status(400).json({
              success: false,
              message: "Bot doesn't have permissions to give verified role",
              code: 500403,
            });
                    if (resp?.status === 404 || resp?.status == 404)
                        return res.status(400).json({
              success: false,
              message: "Verified role not found",
                            code: 500403,
            });
                })
                .catch((err) => {
        ...
        if (server.pulling === true && user.role !== "enterprise")
            return res.status(400).json({
        success: false,
        message: "You are already pulling",
                code: 50055,
      });
        if (server.pullTimeout !== null)
            if (server.pullTimeout > new Date())
                return res.status(400).json({
          success: false,
          message: "You're on cooldown, you can pull again in",
          pullTimeout: server.pullTimeout,
          code: 50056,
        });

        // let done;
        const serverMemberList = await axios.get(
        ...
        if (members.length === 0)
            return res.status(400).json({
        success: false,
        message: "No pullable members found",
                code: 50044,
      });
        ...
        await prisma.servers.update({
            where: { id: server.id },
            data: { pullTimeout: pullTimeout, pulling: true },
        });
        ...
        blacklisted.forEach((blacklistedMember) => {
            const member = members.find(
                (m) => m.userId === (BigInt(blacklistedMember.value) as bigint)
            );
            if (member) {
                members.splice(members.indexOf(member), 1);
                blacklistedCount++;
        ...
        new Promise<void>(async (resolve, reject) => {
            ...
            console.log(
                `[${migration.id}] [${server.name}] Total members: ${
                    members.length
                }, pulling: ${pullCount < members.length ? pullCount : members.length}`
            );
                ...
                await axios
                            ...
                            embeds: [
                                {
                                    ...
                                    author: {
                                        ...
                                        ...(discordServer.icon
                                            ? {
                                                icon_url: `https://cdn.discordapp.com/icons/${discordServer.id}/${discordServer.icon}.png`,
                                            }
                                            : {}),
                                    },
                                    fields: [
                                        {
                                            ...
                                            value: `${
                                                pullCount < members.length ? pullCount : members.length
                                            } members`,
                                            inline: true,
                                        },
                                        {
                    ...
                    .catch(async (err) => {
                        if (err?.response?.status === 404 && server.webhook !== null) {
                            console.error(
                                `${
                                    server.webhook.split("/")[5]
                                } Webhook not found (webhook removed from config)`
                            );
                            ...
                            console.error(
                                `${server?.webhook?.split("/")[5]} Webhook ratelimited`
                            );
                        } else {
                            console.error(err);
                        }
            ...
            for (const member of membersNew) {
                console.log(
                    `[${migration.id}] [${server.name}] [${member.username}] Pulling...`
                );

                if (
                    !(await getMigration(migration.id).then(
                        (m) => m?.status === "PENDING" || m?.status === "PULLING"
                    ))
                ) {
                    console.log(
                        `[${migration.id}] [${server.name}] Migration is not pending or pulling, stopped pulling...`
                    );
                    await prisma.servers
                        .update({
                            where: {
                                id: server.id,
                            },
                        ...
                        .then(() => {
                            console.log(
                                `[${migration.id}] [${server.name}] [PULLING] Set pulling to false`
                            );
                        })
                        .catch((err: Error) => {
                            console.error(
                                `[${migration.id}] [${server.name}] [PULLING] 4 ${err}`
                            );
                        });
                    return reject(
                        "Migration is not pending or pulling, stopped pulling..."
                    );
                }
                ...
                const pullPromise = addMember(
                    guildId.toString(),
                    member.userId.toString(),
                    bot?.botToken,
                    member.accessToken,
                    roleId ? [BigInt(roleId).toString()] : []
                )
                    .then(async (resp: any) => {
                        trysPulled++;
              ...
              resp?.data?.code
                  ...
                  : "";
                        ...
                        console.log(
                            `[${migration.id}] [${server.name}] [${
                                member.username
                            }] ${status} ${JSON.stringify(response).toString() ?? null}`
                        );
                        ...
                        case 429:
                            ...
                            console.log(
                                `[${migration.id}] [${server.name}] [${member.username}] 429 | retry-after: ${retryAfter} | delay: ${delay}ms`
                            );
                            if (retryAfter) {
                            ...
                            break;
                        case 403:
                            if (response?.code === 50025) {
                                // "Invalid OAuth2 access token"
                                const refreshed = await refreshToken(
                                    member.refreshToken,
                                    bot?.clientId.toString(),
                                    bot?.botSecret.toString()
                                );
                                if (
                    ...
                    refreshed?.data?.refresh_token
                                    ...
                                    console.log(
                                        `[${migration.id}] [${server.name}] [${member.username}] Refreshed`
                                    );
                                    await addMember(
                                        guildId.toString(),
                                        member.userId.toString(),
                                        ...
                                        refreshed.data.access_token,
                                        roleId ? [BigInt(roleId).toString()] : []
                                    ).then(async (respon: any) => {
                                        if (
                                            respon?.status === 204 ||
                        respon?.status === 201 ||
                        respon?.response?.status === 204 ||
                        respon?.response?.status === 201
                                        ) {
                                            successCount++;
                                            console.log(
                                                `[${migration.id}] [${server.name}] [${
                                                    ...
                                                    respon?.status || respon?.response?.status
                                                } Refresh PULLED`
                                            );
                                        }
                                    });
                                    ...
                                    console.log(
                                        `[${migration.id}] [${server.name}] [${member.username}] 403 | Refresh Failed`
                                    );
                                    invalidCount++;
                                }
                                ...
                                console.error(
                                    `[${migration.id}] [${server.name}] [${member.username}] 403 | Banned`
                                );
                                bannedCount++;
                            }
                            break;
                        case 407:
                            console.log(
                                `407 Exponential Membership Growth/Proxy Authentication Required`
                            );
                            break;
                        case 204:
                            await addRole(
                        ...
                        case 400:
                            if (response.code === 30001) {
                                ...
                                console.error(
                                    `[FATAL ERROR] [${migration.id}] [${server.name}] [${
                                        member.id
                                    }]-[${member.username}] 400 | ${JSON.stringify(response)}`
                                );
                                errorCount++;
                            }
                            break;
                        case 404:
                            console.error(
                                `[FATAL ERROR] [${migration.id}] [${server.name}] [${
                                    member.id
                                }]-[${member.username}] 404 | ${JSON.stringify(response)}`
                            );
                            errorCount++;
                            break;
                        case 401:
                            console.error(
                                `[${migration.id}] [${server.name}] [${member.id}]-[${member.username}] Bot token invalid stopped pulling...`
                            );
                            reject(`[${server.name}] Bot token invalid stopped pulling...`);
                            break;
                        default:
                            console.error(
                                `[FATAL ERROR] [UNDEFINED STATUS] [${migration.id}] [${
                                    server.name
                                }] [${member.id}]-[${
                                    member.username
                    ...
                    .catch(async (err: Error) => {
                        ...
                        console.error(
                            `[${migration.id}] [${server.name}] [addMember.catch] [${member.username}] ${err}`
                        );
                        errorCount++;
                    })
                    .finally(() => {
                        console.log(
                            `[${migration.id}] [${server.name}] [${member.username}] Pulled`
                        );
                        clearTimeout(pullTimeout);
                        isDone = true;
                    });
                ...
                const pullTimeout = setTimeout(async () => {
                    if (isDone) {
                        ...
                        console.error(
                            `[${migration.id}] [${server.name}] [${
                                member.username
                            }] Pulling timed out after ${
                                MAX_PULL_TIME / 1000
                            } seconds | started: ${Math.round(
                                Number(
                                    new Date(
                                        (await getMigration(migration.id).then(
                                            (m) => m?.startedAt
                ...
                Math.round(
                    Number(
                        new Date(
                            ...
                            }s | [serverId:${serverId} guildId:${guildId} roleId:${roleId} countryCode:${countryCode} server.id:${
                                server.id
                        ...
                        await prisma.servers
                            ...
                            .catch(async (err: Error) => {
                                console.error(
                                    `[${migration.id}] [${server.name}] [PULLING] 5 ${err}`
                                );
                            });
                ...
                if (Number(successCount) >= Number(pullCount)) {
                    console.log(
                        `[${migration.id}] [${server.name}] ${pullCount} members have been pulled`
                    );
                ...
                console.log(
                    `[${migration.id}] [${server.name}] [${
                        ...
                        pullCount !== Number.MAX_SAFE_INTEGER ? pullCount : "∞"
                    } (${trysPulled}/${
                        membersNew.length
                    }) | Errors: ${errorCount} | Delay: ${delay}ms`
                );
            ...
            console.log(
                `[${migration.id}] [${server.name}] [PULLING] Done with ${successCount} members pulled`
            );

            if (await getMigration(migration.id).then((m) => m?.status === "PULLING"))
                await updateMigration(
                    migration.id,
                    "SUCCESS",
                    successCount,
            ...
            .then(async () => {
                ...
                if (
                    await getMigration(migration.id).then((m) => m?.status === "PULLING")
                )
                    await updateMigration(
                        migration.id,
                        "SUCCESS",
                ...
                await prisma.servers
                    ...
                    .catch((err: Error) => {
                        console.error(
                            `[${migration.id}] [${server.name}] [PULLING] 6 ${err}`
                        );
                    });
            })
            .catch((err: Error) => {
      ...
      success: true,
      message: `Started Pull Process, this will take around ${esimatedTime}`,
      time: esimatedTime,
      code: 200,
    });
    } catch (err: any) {
